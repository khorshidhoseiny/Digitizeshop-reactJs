        ...
        {options.map((item) => {
          return (
            <div className="option-select" key={item.value}>
              ...
              />
              <label className="inline-flex items-center justify-center bg-white border-2 border-secondary-color w-9 h-9 rounded-full cursor-pointer ml-4 " htmlFor={item.value}>{item.value}</label>
            </div>
          );
        })}
      </section>
